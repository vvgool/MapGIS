...
//
#include "com_wiesen_test_provider_MapDecodeJNI.h"
#include <iostream>
#include <fstream>
#include "Log.h"
 ...
 */
jbyteArray JNICALL Java_com_wiesen_test_provider_MapDecodeJNI_getTileMap
        (JNIEnv *env, jobject object, jstring path, jint start, jint length){

    const char *j_path = env->GetStringUTFChars(path, 0);
    std::ifstream file(j_path,std::ios::in|std::ios::binary);
    char * map_data = (char *) malloc(length);
    if (!file){
        return NULL;
    }
    file.seekg(start,std::ios_base::beg);
